...
function filterAddonLinterMessageByFile(fileRelPath) {
  return details => {
    return details.file == fileRelPath;
  };
}
...
export async function lint(textEditor, isProject = false) {
  let helpers = require('atom-linter');
  ...
  let warnings = results.warnings
        .filter(filterAddonLinterMessageByFile(isProject ? () => true : fileRelPath))
