...
function isAddonProject(projectDirPath: string) {
...
function filterAddonLinterMessageByFile(isProject: bool, fileRelPath: string) {
...
export async function sanityCheckEnvironment(): Promise<bool> {
...
export async function lint(
  textEditor: TextEditor, isProject: bool = false
): Promise<LinterMessages> {
  const helpers = require('atom-linter'); // eslint-disable-line global-require
  ...
  const out = await helpers.exec(webextExecutablePath, ["lint", "-o", "json"], {
    cwd: projectDirPath,
    ignoreExitCode: true,
...
export function subscribeCommands(): Disposable {
  return atom.commands.add('atom-workspace', {
    'web-ext:force-lint-on-this-project': () => {
      addonsLinterForceOnProject();
    },
  });
}

export function provideLinter(): LinterInfo {
